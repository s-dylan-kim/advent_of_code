...
int main(void)
{
    ...
    while (getline(file, in)) {
        ...
        visited.push_back(vector<vector<vector<int>>>(curLine.size(), vector<vector<int>>(4, vector<int>(3, -1))));
    }
    ...
    toVisit.push(DjikstrasNode(0, 1, East, 1, weights[0][1]));
    toVisit.push(DjikstrasNode(1, 0, South, 1, weights[1][0]));
    visited[0][1][East][1] = weights[0][1];
    visited[1][0][South][1] = weights[1][0];
    ...
    while (!toVisit.empty()) {
        ...
        if (cur.x == ((int)weights[0].size() - 1) && cur.y == ((int)weights.size() - 1)) {
            sol = cur.totalDist;
            break;
        }

        if ((cur.dir == North && cur.walkCnt < 2) || (cur.dir == East || cur.dir == West)) {
            int newWalkCnt = 0;
            ...
            if (cur.y > 0) {
                ...
                for (int i = 0; i <= newWalkCnt; i++) {
                    if (visited[cur.y - 1][cur.x][North][i] <= newDist && visited[cur.y - 1][cur.x][North][i] != -1) {
                        toAdd = false;
                    }
                }
                if (toAdd) {
        ...
        if ((cur.dir == South && cur.walkCnt < 2) || (cur.dir == East || cur.dir == West)) {
            int newWalkCnt = 0;
        ...
        if ((cur.dir == East && cur.walkCnt < 2) || (cur.dir == North || cur.dir == South)) {
            int newWalkCnt = 0;
        ...
        if ((cur.dir == West && cur.walkCnt < 2) || (cur.dir == North || cur.dir == South)) {
            int newWalkCnt = 0;
