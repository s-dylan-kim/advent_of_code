...
int main(void)
{
    ...
    while (file >> hand >> val) {
        ...
        // cout  << val << ", " << largestMatch << endl;

        largestMatch += jokerCnt;

        if (largestMatch > 5) largestMatch = 5;
        ...
        // if (handCopy.find('J') != string::npos) cout << handCopy << ", " << handRank << endl;
    ...
    for (int i = 0; i < hands.size(); i++) {
        // cout << (long)get<1>(hands[i]) << ", " << (long)get<2>(hands[i]) << endl;
